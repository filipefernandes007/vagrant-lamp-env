...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision :shell, inline: <<-SCRIPT
    ...
    config.vm.provision :shell, path: "scripts/bootstrap.sh", env: {"HOMEDIRFOLDER" => parse['home_dir_folder']}
    ...
    config.vm.provision :shell, privileged: true, path: "scripts/setup-apache2.sh", env: {"HOMEDIRFOLDER" => parse['home_dir_folder']}

    if parse['dependencies']['php-71'] == true
    ...
    config.vm.provision :shell, privileged: true, path: "scripts/composer.sh"
